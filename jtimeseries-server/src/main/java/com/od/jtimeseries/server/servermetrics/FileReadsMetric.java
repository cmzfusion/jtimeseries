 ...
 * Time: 09:08
 */
public class FileReadsMetric extends AbstractManagedMetric {
    private static final String id = "FileReads";
    private String parentContextPath;
    private TimePeriod captureTime;

    public FileReadsMetric(String parentContextPath) {
        this(parentContextPath, DEFAULT_TIME_PERIOD_FOR_SERVER_METRICS);
    }

    public FileReadsMetric(String parentContextPath, TimePeriod captureTime) {
    ...
    public void doInitializeMetric(TimeSeriesContext rootContext, String path) {
        Counter c = rootContext.createCounterSeries(path, "Count of series data file read operations", COUNT_OVER(captureTime), LATEST_COUNT(captureTime));
        RoundRobinSerializer.setFileReadCounter(c);
    }
}
