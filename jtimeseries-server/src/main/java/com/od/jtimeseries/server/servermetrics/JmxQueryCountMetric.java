...
public class JmxQueryCountMetric extends AbstractManagedMetric {
    ...
    public void doInitializeMetric(TimeSeriesContext rootContext, String path) {
        Counter c = rootContext.createCounterSeries(path, "Number of queries operations made to open jmx conections, several values may be read during each query", COUNT_OVER(captureTime), TOTAL_COUNT(captureTime));
