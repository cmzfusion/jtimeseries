 ...
 *
 * The reason we need this is that we require the data structures for list based series to meet the
 * following criteria:
 *
 * - adding to end of list is very fast
...
class RandomAccessDeque<E> extends AbstractCollection<E> implements Deque<E>, Serializable, RandomAccess, ModCount {
    ...
    /**
     * Used to check for comodification during iteration
     */
     ...
     */
    public RandomAccessDeque(int numElements) {
     ...
     */
    public RandomAccessDeque(Collection<? extends E> c) {
    ...
    public E get(int index) {
        if ( index < 0 || index >= size()) {
            throw new IndexOutOfBoundsException("RandomAccessDeque get index " + index + " out of range");
        } else {
            int i = getArrayIndexForElementAt(index);
            return elements[i];
        }
    }

    private int getArrayIndexForElementAt(int index) {
        //n.b. this is the same as (index + head) % elements.length since elements.length is a power of 2
        return (index + head) & elements.length - 1;
    }
    ...
    public E remove(int index) {
        if ( index < 0 || index >= size()) {
            throw new IndexOutOfBoundsException("Index " + index + " out of bounds, current size " + size());
        }

        E result = get(index);
        for ( int loop=index + 1; loop < size(); loop++) {
            set(loop -1, get(loop));
        }
        removeLast();
        return result;
